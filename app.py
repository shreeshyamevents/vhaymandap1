...
def create_notification(user_id, ntype, title, message, link=None):
    """Batch 5 — unified notification helper. Caller commits."""
    if not user_id:
        return None
    try:
...
def vendor_earnings():
    ...
    total_earnings = round(sum((b.base_rent or 0) for b in completed), 2)
    ...
    for b in completed:
        ...
        if key in monthly:
            monthly[key]['amount'] += (b.base_rent or 0)
            monthly[key]['count'] += 1
