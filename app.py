...
def calculate():
    ...
    if start_date < today_date:
        return jsonify({'error': True, 'message': 'Start date cannot be in the past.'}), 400
    if end_date < start_date:
        return jsonify({'error': True, 'message': 'End date must be on or after start date.'}), 400
    if quantity <= 0:
        return jsonify({'error': True, 'message': 'Quantity must be at least 1.'}), 400
    if (end_date - start_date).days > 365:
        return jsonify({'error': True, 'message': 'Booking period cannot exceed 365 days.'}), 400
    if area not in ['city', 'outskirts']:
        return jsonify({'error': True, 'message': 'Invalid delivery area.'}), 400
    if weight not in ['till_20', 'above_20']:
        return jsonify({'error': True, 'message': 'Invalid weight category.'}), 400
