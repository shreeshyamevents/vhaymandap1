...
# ============================================
# CLOUDINARY PHOTO UPLOAD HELPERS
# ============================================

def upload_base64_to_cloudinary(base64_string, folder='vyahmandap/reports'):
    """Upload base64 image to Cloudinary. Returns URL or None."""
...
def get_client_info(request_obj):
    """Extract device info, IP from request"""
        ...
        try:
            ...
            if item.vendor.telegram_chat_id:
                vendor_msg = (
                    ...
                    f"⚠️ Upload equipment condition report before dispatching!"
                )
...
def dashboard():
    ...
    # Bookings needing return report (action required)
    ...
    if report_type == 'dispatch':
        if booking.customer.telegram_chat_id:
            msg = (
                ...
                f"Condition Report uploaded: {len(uploaded)} photos\n\n"
        ...
        if booking.item.vendor.telegram_chat_id:
            msg = (
                ...
                f"Item: {booking.item.title}\n"
                f"Condition Report uploaded: {len(uploaded)} photos\n\n"
                f"Please review the return photos. 🔍"
            )
...
def vendor_dashboard():
    ...
    # Bookings needing dispatch report
